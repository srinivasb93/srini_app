...
from schemas import (
    ...
    HistoricalDataResponse, Instrument, OrderHistory, Trade, ModifyOrderRequest
)
...
async def startup_event():
    # Initialize the database engine before starting any tasks
    session_factory, _ = await init_engine()
    logger.info("Database engine and session factory initialized successfully")

    # Now that the engine is initialized, start the background tasks
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logger.info(f"Token expired for user {user.user_id} during login, proceeding with authentication")
...
@app.get("/margins/{broker}", response_model=MarginResponse, tags=["portfolio"])
async def get_margins(broker: str, user_id: str = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        if broker == "Upstox":
            margins = api.get_user_fund_margin(api_version="v2").data
            response = MarginResponse(
                equity={
                    "available": margins.equity.available_margin if margins.equity else 0,
                    "used": margins.equity.used_margin if margins.equity else 0
                },
                commodity={
                    "available": margins.commodity.available_margin if margins.commodity else 0,
                    "used": margins.commodity.used_margin if margins.commodity else 0
                },
                broker=broker
            )
        else:
...
async def get_order_details(broker: str, order_id: str, user_id: str = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        history = get_order_history(upstox_api, kite_api, order_id, broker)
        return history
    except Exception as e:
...
async def get_trades_for_order(broker: str, order_id: str, user_id: str = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        trades = get_order_trades(upstox_api, kite_api, order_id, broker)
        return trades
    except Exception as e:
...
async def get_order_book_data(broker: str, user_id: str = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        order_book = get_order_book(upstox_api, kite_api)
...
async def get_gtt_orders(broker: str, user_id: str = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        if broker == "Zerodha":
            ...
            # For Upstox, fetch from DB as Upstox doesn't support GTT natively
...
async def delete_gtt_order(broker: str, gtt_id: str, user_id: str = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        response = await order_manager.delete_gtt_order(api, gtt_id, db)
        return response
    except Exception as e:
...
async def get_positions_data(broker: str, user_id: str = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        positions = get_positions(upstox_api, zerodha_api)
...
async def get_portfolio_data(broker: str, user_id: str = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        portfolio = get_portfolio(upstox_api, zerodha_api)
...
async def get_funds(broker: str, user_id: str = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        funds_data = get_funds_data(api, broker)
        return funds_data
    except Exception as e:
...
async def get_market_quotes(broker: str, instruments: str, user_id: str = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        quotes = get_quotes(upstox_api, kite_api, instrument_list)
        return quotes
    except Exception as e:
...
async def get_ohlc_data(broker: str, instruments: str, user_id: str = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        instrument_list = instruments.split(",")
        ohlc_data = get_ohlc(upstox_api, kite_api, instrument_list)
        return ohlc_data
    except Exception as e:
...
async def get_ltp_data(broker: str, instruments: str, user_id: str = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        instrument_list = instruments.split(",")
        #  Todo: Check if Upstox supports LTP for multiple instruments
        ltp_data = get_ltp(upstox_api, upstox_api, instrument_list)
        return ltp_data
    except Exception as e:
...
async def get_historical(broker: str, instrument: str, from_date: str, to_date: str, interval: str,
    ...
    user_apis_dict = await initialize_user_apis(user_id, db)
    upstox_api = user_apis_dict["upstox"]["history"] if broker == "Upstox" else None  # Use history API for Upstox
    kite_api = user_apis_dict["zerodha"]["kite"] if broker == "Zerodha" else None
    if not (upstox_api or kite_api):
        raise HTTPException(status_code=400, detail=f"{broker} API not initialized")

    try:
        historical_data = get_historical_data(upstox_api, kite_api, instrument, from_date, to_date, interval)
...
async def get_instruments_list(broker: str, exchange: Optional[str] = None, user_id: str = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        instruments = get_instruments(upstox_api, kite_api, exchange)
        return instruments
    except Exception as e:
...
@app.get("/mutual-funds/sips",  tags=["mutual-funds"])
